...
```
generate data → extract data → first validation → transform data → load data → log metadata →
→ export bigquery → second validation → export bigquery → export googlesheets
```
